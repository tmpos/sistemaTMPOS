...
/************************************************************************/
const camposArray = ["texto","usuario"];
/************************************************************************/
...
const MensajesEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposMensajes.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
const response = await peticionesFetchOffline('getDataAsArray', 'mensajes');
    const jsonData = response;
    data.value = jsonData;
};
/************************************************************************/
async function campos() {
  const campos = await arrayToObjetoFromTablaOffline('mensajes');
  datoscamposMensajes.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExisteOffline('mensajes', camposArray, toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'mensajes');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/mensajes";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'mensajes', JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
    visible.value = false;
    fetchAndSetupData();
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/mensajes";
  if (datoscamposMensajes.value.hasOwnProperty('created_at')) {
    datoscamposMensajes.value.created_at = nfecha('timestamp');
    datoscamposMensajes.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await peticionesFetchOffline('insertData', 'mensajes', JSON.stringify(datoscamposMensajes.value));
  if (envioDatos[0] == 'ok') {
    await fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await peticionesFetchOffline('deleteEntry', 'mensajes', id);
                            } catch (error) {
...
/************************************************************************/
const itemsMensajes = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleMensajes = (event, rowData) => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry', 'mensajes', rowData.id);
...
<div class="grid grid-cols-12 gap-4">
      <div class="md:col-span-12">
        <fieldset class="border p-3 round mb-2">
          <legend class="float-none w-auto px-2">Datos de Mensajes</legend>
            <div class="grid grid-cols-12 gap-4">
...
<Button icon="pi pi-trash" v-if="usuarioLocal.usuario =='Soporte'" raised round severity="danger" label="Borrar Todo" class="float-end" @click="borrarTodo" aria-label="Borrar Todo" />
 ...
 <input v-model="searchQuery" placeholder="Buscar mensajes..." class="p-inputtext p-component" style="margin-bottom: 10px;" />
</div>
<DataTable 
    :value="filteredMensajes"  
    scrollable 
    scrollHeight="600px"  
    dataKey="id" 
    paginator 
    ...
    <Column header="Options">
        <template #body="slotProps">
            <Button 
                icon="pi pi-cog" 
                @click="toggleMensajes($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
            />
            <Menu 
                ref="menu" 
                id="overlay_menu_Mensajes" 
                :model="itemsMensajes" 
                :popup="true"
            />
        </template>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Mensajes" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
  ...
  <fieldset class="border p-3 round mb-2">
    <legend class="float-none w-auto px-2">Mensajes</legend>
     <form id="formularioActualizarMensajes" action="" method="">
...
<label for="texto-Actualizador">TEXTO</label>
<textarea class="form-control " id="texto-Actualizador" name="texto" v-model="datoscampos.texto" cols="30" rows="3" ></textarea>
</div>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Mensajes" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="inline-flex align-items-center justify-content-center gap-2">
  ...
  <fieldset class="border p-3 round mb-2">
    <legend class="float-none w-auto px-2">Mensajes</legend>
     <form id="formularioActualizarMensajes" action="" method="">
         <div  style="margin-top: 15px;color: #34AAB2;" class="grid grid-cols-12 gap-4">
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="textoAgregarDatos">TEXTO</label>
<textarea class="form-control " v-model="datoscamposMensajes.texto" id="textoAgregarDatos" name="texto" cols="30" rows="3" ></textarea>
</div>
<div class="form-group col-span-6" hidden>
<label for="created_atAgregarDatos">CREATED_AT</label>
<input type="input" v-model="datoscamposMensajes.created_at" name="created_at"  class="form-control " id="created_atAgregarDatos"   placeholder="created_at" maxlength="">
</div>
<div class="form-group col-span-6" hidden>
<label for="updated_atAgregarDatos">UPDATED_AT</label>
<input type="input" v-model="datoscamposMensajes.updated_at" name="updated_at"  class="form-control " id="updated_atAgregarDatos"   placeholder="updated_at" maxlength="">
</div>
<div class="form-group col-span-12" hidden>
<label for="usuarioAgregarDatos">USUARIO</label>
<input type="input" v-model="datoscamposMensajes.usuario" name="usuario"  class="form-control " id="usuarioAgregarDatos"   placeholder="usuario" maxlength="250">
</div>
