...
import { enviarDatosPorPost, eliminarDatos, obtenerIdsSeleccionados, borrarTodoslosDatos, lenguajeDataTable, nfecha, arrayToObjetoFromTabla, peticionesFetch, encryptarPassword, envioElectron, mensajetoast, generarCodigoUnico, peticiones, lasMayusculas, peticionesFetchOffline, arrayToObjetoFromTablaOffline, crearTablaSiNoExisteOffline } from '../../funciones/funciones.js';
...
const response = await peticionesFetchOffline('getDataAsArray', 'whatsapp');
    const jsonData = response;
    data.value = jsonData;
};
/************************************************************************/
async function campos(){
  const campos = await arrayToObjetoFromTablaOffline('whatsapp');
...
const envioDatos = await peticionesFetchOffline('deleteAll', 'whatsapp');
  if (envioDatos[0] == 'ok') {
     fetchAndSetupData();
...
const envioDatos = await peticionesFetchOffline('updateData', 'whatsapp', JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
     visible.value = false
     fetchAndSetupData();
...
async function funcionCrear(){
  ...
  const envioDatos = await peticionesFetchOffline('insertData', 'whatsapp', JSON.stringify(datoscamposWhatsapp.value));
  if (envioDatos[0] == 'ok') {
    await uploadImages()
     fetchAndSetupData();
...
let exitoTotal = true; 
    if (ids.length > 0) {
        for (const id of ids) {
            try {
               const envioDatos = await peticionesFetchOffline('deleteEntry', 'whatsapp', id);
            } catch (error) {
...
<div class="grid grid-cols-12 gap-4">
 ...
 <div class="border-1 surface-border border-round m-2 col-span-12 md:col-span-3 p-3" v-for="imagen in arrayIMG">
     <div class="mb-3">
