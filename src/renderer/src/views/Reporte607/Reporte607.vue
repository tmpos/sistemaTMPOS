<script setup>
import { ref, computed, onMounted } from 'vue';
import { useRouter } from 'vue-router';
import { useToast } from "primevue/usetoast";
import Swal from 'sweetalert2';
import {
  envioElectron,
  encryptarPassword,
  nfecha,
  formatearFecha,
  peticionesFetchOffline
} from '@/funciones/funciones.js';
import { useDatosEmpresa } from '@/stores';
import * as XLSX from 'xlsx';
import { jsPDF } from 'jspdf';
import 'jspdf-autotable';

const router = useRouter();
const toast = useToast();
const datosEmpresa = useDatosEmpresa();

const link = ref('');
const api = ref('');
const token = ref('');
const tokenCifrado = ref(null);
const loading = ref(false);

// Filtros
const rangoFecha = ref([]);
const busqueda = ref('');
const tipoNCFFilter = ref('');
const tipoComprobanteFilter = ref('');

// Datos
const ventasData = ref([]);
const ventasFiltradas = computed(() => {
  let filtered = ventasData.value;

  // Filtro por búsqueda
  if (busqueda.value) {
    const query = busqueda.value.toLowerCase();
    filtered = filtered.filter(item =>
      item.nombre_cliente?.toLowerCase().includes(query) ||
      item.cod_cliente?.toLowerCase().includes(query) ||
      item.no_factura?.toLowerCase().includes(query) ||
      item.comprobante?.toLowerCase().includes(query)
    );
  }

  // Filtro por tipo de comprobante
  if (tipoComprobanteFilter.value) {
    filtered = filtered.filter(item => item.comprobante === tipoComprobanteFilter.value);
  }

  return filtered;
});

// Estadísticas
const totalVentas = computed(() => ventasFiltradas.value.length);
const totalMonto = computed(() => {
  return ventasFiltradas.value.reduce((sum, item) => sum + parseFloat(item.total || 0), 0);
});
const totalITBIS = computed(() => {
  return ventasFiltradas.value.reduce((sum, item) => sum + parseFloat(item.impuesto || 0), 0);
});
const totalBase = computed(() => {
  return ventasFiltradas.value.reduce((sum, item) => sum + parseFloat(item.subtotal || 0), 0);
});

// Tipos de comprobantes según normativa DGII
const tiposComprobantes = [
  { label: 'Todos', value: '' },
  { label: 'Consumidor Final', value: 'CONSUMIDOR FINAL' },
  { label: 'Comprobante Fiscal', value: 'COMPROBANTE FISCAL' },
  { label: 'Régimen Especial', value: 'REGIMEN ESPECIAL' },
  { label: 'Gubernamental', value: 'COMPROBANTE GUBERNAMENTAL' },
  { label: 'Sin Comprobante', value: 'SIN COMPROBANTE' }
];

/************************************************************************/
document.body.classList.add('sidebar-close');

/************************************************************************/
onMounted(async() => {
  try {
    const datosJSON = await envioElectron('datosarchivo');
    link.value = datosJSON.VITE_LINKURL;
    api.value = datosJSON.VITE_LINK_API;
    token.value = datosJSON.VITE_TOKEN;

    tokenCifrado.value = await encryptarPassword(token.value, 10);

    if (!datosEmpresa.empresa.nombre) {
      await datosEmpresa.inicializarDatosEmpresa(link.value + api.value);
    }

    // Establecer fechas del mes actual por defecto
    const hoy = new Date();
    const primerDia = new Date(hoy.getFullYear(), hoy.getMonth(), 1);
    const ultimoDia = new Date(hoy.getFullYear(), hoy.getMonth() + 1, 0);
    rangoFecha.value = [primerDia, ultimoDia];

    await cargarDatos();
  } catch (error) {
    console.error('Error al inicializar:', error);
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Error al cargar configuración',
      life: 3000
    });
  }
});

/************************************************************************/
// Función helper para parsear fechas DD/MM/YYYY
const parsearFecha = (fechaStr) => {
  if (!fechaStr) return null;

  // Formato DD/MM/YYYY (usado por nfecha('fecha'))
  if (fechaStr.includes('/')) {
    const partes = fechaStr.split('/');
    if (partes.length === 3) {
      const dia = parseInt(partes[0], 10);
      const mes = parseInt(partes[1], 10) - 1;
      const año = parseInt(partes[2], 10);
      const fecha = new Date(año, mes, dia);
      fecha.setHours(0, 0, 0, 0);
      return fecha;
    }
  }

  // Formato YYYY-MM-DD
  if (fechaStr.includes('-')) {
    const fecha = new Date(fechaStr);
    fecha.setHours(0, 0, 0, 0);
    return fecha;
  }

  return null;
};

const cargarDatos = async () => {
  if (!rangoFecha.value || rangoFecha.value.length < 2) {
    toast.add({
      severity: 'warn',
      summary: 'Advertencia',
      detail: 'Selecciona un rango de fechas válido',
      life: 3000
    });
    return;
  }

  loading.value = true;
  try {
    // Obtener todas las facturas
    const response = await peticionesFetchOffline('getDataAsArray', 'facturas');
    console.log('Total facturas en DB:', response?.length || 0);

    // Normalizar rango de fechas
    const fechaInicio = new Date(rangoFecha.value[0]);
    fechaInicio.setHours(0, 0, 0, 0);
    const fechaFin = new Date(rangoFecha.value[1]);
    fechaFin.setHours(23, 59, 59, 999);

    console.log('Rango seleccionado:', {
      inicio: fechaInicio.toLocaleDateString('es-DO'),
      fin: fechaFin.toLocaleDateString('es-DO')
    });

    ventasData.value = (response || []).filter(factura => {
      // FILTRO 1: Solo facturas con comprobante fiscal válido
      if (!factura.comprobante ||
          factura.comprobante.trim() === '' ||
          factura.comprobante === 'SIN COMPROBANTE') {
        return false;
      }

      // FILTRO 2: Verificar y parsear fecha
      if (!factura.fecha_emision) return false;

      const fechaFactura = parsearFecha(factura.fecha_emision);
      if (!fechaFactura) {
        console.warn('Fecha inválida:', factura.fecha_emision, factura.no_factura);
        return false;
      }

      return fechaFactura >= fechaInicio && fechaFactura <= fechaFin;
    }).map((factura, index) => ({
      ...factura,
      linea: index + 1,
      rnc_cedula: extraerRNCCedula(factura.cod_cliente),
      montoFacturado: parseFloat(factura.total || 0),
      itbisFacturado: parseFloat(factura.impuesto || 0),
      baseImponible: parseFloat(factura.subtotal || 0)
    }));

    console.log('Ventas con comprobante filtradas:', ventasData.value.length);

    toast.add({
      severity: 'success',
      summary: 'Éxito',
      detail: `${ventasData.value.length} ventas con comprobante cargadas`,
      life: 3000
    });
  } catch (error) {
    console.error('Error al cargar datos:', error);
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Error al cargar datos de ventas',
      life: 3000
    });
  } finally {
    loading.value = false;
  }
};

const cargarTodasLasVentas = async () => {
  loading.value = true;
  try {
    const response = await peticionesFetchOffline('getDataAsArray', 'facturas');
    const totalFacturas = response?.length || 0;

    // Filtrar solo facturas con comprobante fiscal válido
    const facturasConComprobante = (response || []).filter(factura =>
      factura.comprobante &&
      factura.comprobante.trim() !== '' &&
      factura.comprobante !== 'SIN COMPROBANTE'
    );

    ventasData.value = facturasConComprobante.map((factura, index) => ({
      ...factura,
      linea: index + 1,
      rnc_cedula: extraerRNCCedula(factura.cod_cliente),
      montoFacturado: parseFloat(factura.total || 0),
      itbisFacturado: parseFloat(factura.impuesto || 0),
      baseImponible: parseFloat(factura.subtotal || 0)
    }));

    console.log(`Total facturas: ${totalFacturas}, Con comprobante: ${facturasConComprobante.length}`);

    toast.add({
      severity: 'success',
      summary: 'Ventas con Comprobante',
      detail: `${ventasData.value.length} ventas con comprobante cargadas (de ${totalFacturas} totales)`,
      life: 4000
    });
  } catch (error) {
    console.error('Error al cargar todas las ventas:', error);
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Error al cargar ventas',
      life: 3000
    });
  } finally {
    loading.value = false;
  }
};

/************************************************************************/
const exportarExcel = () => {
  if (ventasFiltradas.value.length === 0) {
    toast.add({
      severity: 'warn',
      summary: 'Advertencia',
      detail: 'No hay datos para exportar',
      life: 3000
    });
    return;
  }

  const datos = ventasFiltradas.value.map((item, index) => ({
    'Línea': index + 1,
    'RNC/Cédula': item.rnc_cedula || '',
    'Tipo Identificación': item.rnc_cedula ? (item.rnc_cedula.length === 9 ? 'RNC' : 'Cédula') : '',
    'Nombre/Razón Social': item.nombre_cliente || '',
    'Tipo Comprobante': obtenerTipoComprobante(item.comprobante),
    'NCF': item.tipo_factura || '',
    'No. Factura': item.no_factura || '',
    'Fecha': formatearFecha(item.fecha_emision),
    'Monto Facturado': parseFloat(item.total || 0).toFixed(2),
    'ITBIS Facturado': parseFloat(item.impuesto || 0).toFixed(2),
    'ITBIS Retenido': '0.00',
    'ITBIS Percibido': '0.00',
    'Retención Renta': '0.00',
    'ISR Percibido': '0.00',
    'Base Imponible': parseFloat(item.subtotal || 0).toFixed(2),
    'Propina Legal': '0.00',
    'Efectivo': parseFloat(item.efectivo || 0).toFixed(2),
    'Cheque/Transferencia': (parseFloat(item.cheque || 0) + parseFloat(item.transferencia || 0)).toFixed(2),
    'Tarjeta': parseFloat(item.tarjeta || 0).toFixed(2)
  }));

  const ws = XLSX.utils.json_to_sheet(datos);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, '607');

  // Ajustar anchos de columna
  const colWidths = [
    { wch: 8 },  // Línea
    { wch: 15 }, // RNC
    { wch: 12 }, // Tipo ID
    { wch: 35 }, // Nombre
    { wch: 20 }, // Tipo
    { wch: 20 }, // NCF
    { wch: 15 }, // No. Factura
    { wch: 12 }, // Fecha
    { wch: 15 }, // Monto
    { wch: 15 }, // ITBIS
    { wch: 15 }, // ITBIS Ret
    { wch: 15 }, // ITBIS Perc
    { wch: 15 }, // Ret Renta
    { wch: 15 }, // ISR Perc
    { wch: 15 }, // Base
    { wch: 12 }, // Propina
    { wch: 15 }, // Efectivo
    { wch: 15 }, // Cheque/Trans
    { wch: 15 }  // Tarjeta
  ];
  ws['!cols'] = colWidths;

  const periodo = `${formatearFecha(rangoFecha.value[0])}_${formatearFecha(rangoFecha.value[1])}`;
  XLSX.writeFile(wb, `607_Ventas_${periodo}.xlsx`);

  toast.add({
    severity: 'success',
    summary: 'Éxito',
    detail: 'Archivo Excel exportado correctamente',
    life: 3000
  });
};

/************************************************************************/
const exportarTXT = () => {
  if (ventasFiltradas.value.length === 0) {
    toast.add({
      severity: 'warn',
      summary: 'Advertencia',
      detail: 'No hay datos para exportar',
      life: 3000
    });
    return;
  }

  // Formato TXT para DGII según normativa 607
  const lineas = ventasFiltradas.value.map(item => {
    const campos = [
      item.rnc_cedula || '',                              // RNC/Cédula
      item.rnc_cedula ? (item.rnc_cedula.length === 9 ? '1' : '2') : '1', // Tipo ID (1=RNC, 2=Cédula)
      obtenerCodigoTipoComprobante(item.comprobante),    // Tipo Comprobante
      item.tipo_factura || '',                            // NCF
      '',                                                  // NCF Modificado (opcional)
      formatearFechaDGII(item.fecha_emision),             // Fecha Comprobante
      formatearMonto(item.total || 0),                    // Monto Facturado
      formatearMonto(item.impuesto || 0),                 // ITBIS Facturado
      formatearMonto(0),                                   // ITBIS Retenido
      formatearMonto(0),                                   // ITBIS Percibido
      formatearMonto(0),                                   // Retención Renta
      formatearMonto(0),                                   // ISR Percibido
      formatearMonto(item.subtotal || 0),                 // Base Imponible
      formatearMonto(0),                                   // Propina Legal
      formatearMonto(item.efectivo || 0),                 // Efectivo
      formatearMonto((parseFloat(item.cheque || 0) + parseFloat(item.transferencia || 0))), // Cheque/Transferencia
      formatearMonto(item.tarjeta || 0),                  // Tarjeta
      formatearMonto(0)                                    // Otras formas de pago
    ];
    return campos.join('|');
  });

  const contenido = lineas.join('\n');
  const blob = new Blob([contenido], { type: 'text/plain' });
  const url = window.URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  const periodo = `${formatearFecha(rangoFecha.value[0])}_${formatearFecha(rangoFecha.value[1])}`;
  a.download = `607_${datosEmpresa.empresa.rnc || 'SIN_RNC'}_${periodo}.txt`;
  a.click();
  window.URL.revokeObjectURL(url);

  toast.add({
    severity: 'success',
    summary: 'Éxito',
    detail: 'Archivo TXT exportado correctamente',
    life: 3000
  });
};

/************************************************************************/
const exportarPDF = () => {
  if (ventasFiltradas.value.length === 0) {
    toast.add({
      severity: 'warn',
      summary: 'Advertencia',
      detail: 'No hay datos para exportar',
      life: 3000
    });
    return;
  }

  const doc = new jsPDF('l', 'mm', 'a4');

  // Encabezado
  doc.setFontSize(16);
  doc.text('REPORTE 607 - VENTAS', 148, 15, { align: 'center' });

  doc.setFontSize(10);
  doc.text(`Empresa: ${datosEmpresa.empresa.nombre || 'N/A'}`, 14, 25);
  doc.text(`RNC: ${datosEmpresa.empresa.rnc || 'N/A'}`, 14, 30);
  doc.text(`Periodo: ${formatearFecha(rangoFecha.value[0])} - ${formatearFecha(rangoFecha.value[1])}`, 14, 35);
  doc.text(`Fecha de emisión: ${formatearFecha(new Date())}`, 14, 40);

  // Tabla
  const tableData = ventasFiltradas.value.map((item, index) => [
    index + 1,
    item.rnc_cedula || 'N/A',
    item.nombre_cliente || '',
    obtenerTipoComprobante(item.comprobante),
    item.tipo_factura || '',
    item.no_factura || '',
    formatearFecha(item.fecha_emision),
    `RD$ ${parseFloat(item.total || 0).toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
    `RD$ ${parseFloat(item.impuesto || 0).toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  ]);

  doc.autoTable({
    startY: 45,
    head: [['#', 'RNC/Céd', 'Cliente', 'Tipo', 'NCF', 'No. Fact', 'Fecha', 'Total', 'ITBIS']],
    body: tableData,
    theme: 'grid',
    styles: { fontSize: 8, cellPadding: 2 },
    headStyles: { fillColor: [22, 160, 133], textColor: 255, fontStyle: 'bold' },
    columnStyles: {
      0: { cellWidth: 10 },
      1: { cellWidth: 25 },
      2: { cellWidth: 50 },
      3: { cellWidth: 30 },
      4: { cellWidth: 30 },
      5: { cellWidth: 25 },
      6: { cellWidth: 25 },
      7: { cellWidth: 30, halign: 'right' },
      8: { cellWidth: 25, halign: 'right' }
    },
    didDrawPage: (data) => {
      // Pie de página
      doc.setFontSize(8);
      doc.text(`Página ${doc.internal.getCurrentPageInfo().pageNumber}`, 148, doc.internal.pageSize.height - 10, { align: 'center' });
    }
  });

  // Totales
  const finalY = doc.lastAutoTable.finalY + 10;
  doc.setFontSize(10);
  doc.setFont(undefined, 'bold');
  doc.text('RESUMEN:', 14, finalY);
  doc.setFont(undefined, 'normal');
  doc.text(`Total de ventas: ${totalVentas.value}`, 14, finalY + 5);
  doc.text(`Monto total: RD$ ${totalMonto.value.toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 14, finalY + 10);
  doc.text(`ITBIS total: RD$ ${totalITBIS.value.toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 14, finalY + 15);
  doc.text(`Base imponible: RD$ ${totalBase.value.toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 14, finalY + 20);

  // Mostrar PDF embebido en SweetAlert
  const pdfBlob = doc.output('blob');
  const pdfUrl = URL.createObjectURL(pdfBlob);
  const periodo = `${formatearFecha(rangoFecha.value[0])}_${formatearFecha(rangoFecha.value[1])}`;

  Swal.fire({
    title: 'Reporte 607 - Ventas',
    html: `
      <div style="width: 100%; height: 70vh;">
        <iframe
          src="${pdfUrl}"
          style="width: 100%; height: 100%; border: none; border-radius: 8px;"
          title="Reporte 607 PDF"
        ></iframe>
      </div>
    `,
    width: '90%',
    showCloseButton: true,
    showConfirmButton: true,
    confirmButtonText: '<i class="pi pi-download"></i> Descargar PDF',
    confirmButtonColor: '#16a085',
    didOpen: () => {
      const closeButton = Swal.getCloseButton();
      if (closeButton) {
        closeButton.addEventListener('click', () => {
          URL.revokeObjectURL(pdfUrl);
        });
      }
    },
    preConfirm: () => {
      const link = document.createElement('a');
      link.href = pdfUrl;
      link.download = `607_Ventas_${periodo}.pdf`;
      link.click();
      return false;
    }
  }).then((result) => {
    URL.revokeObjectURL(pdfUrl);
  });
};

/************************************************************************/
// Funciones auxiliares
const extraerRNCCedula = (codCliente) => {
  // Intentar extraer RNC/Cédula del código de cliente
  // Formato esperado: "nombre (RNC)" o similar
  if (!codCliente) return '';
  const match = codCliente.match(/\(([0-9-]+)\)/);
  return match ? match[1] : '';
};

const obtenerTipoComprobante = (comprobante) => {
  if (!comprobante) return 'Sin Comprobante';
  const tipos = {
    'CONSUMIDOR FINAL': '02 - Consumidor Final',
    'COMPROBANTE FISCAL': '01 - Crédito Fiscal',
    'REGIMEN ESPECIAL': '14 - Régimen Especial',
    'COMPROBANTE GUBERNAMENTAL': '15 - Gubernamental',
    'SIN COMPROBANTE': 'Sin Comprobante'
  };
  return tipos[comprobante] || comprobante;
};

const obtenerCodigoTipoComprobante = (comprobante) => {
  const codigos = {
    'CONSUMIDOR FINAL': '02',
    'COMPROBANTE FISCAL': '01',
    'REGIMEN ESPECIAL': '14',
    'COMPROBANTE GUBERNAMENTAL': '15',
    'SIN COMPROBANTE': ''
  };
  return codigos[comprobante] || '';
};

const formatearFechaDGII = (fecha) => {
  if (!fecha) return '';
  const d = new Date(fecha);
  const year = d.getFullYear();
  const month = String(d.getMonth() + 1).padStart(2, '0');
  const day = String(d.getDate()).padStart(2, '0');
  return `${year}${month}${day}`;
};

const formatearMonto = (monto) => {
  return parseFloat(monto || 0).toFixed(2);
};
</script>

<template>
  <main class="content-wrapper bg-gray-50 min-h-screen">
    <div class="w-full px-4 py-6">
      <!-- Header -->
      <div class="mb-6">
        <div class="flex items-center justify-between mb-2">
          <div>
            <h1 class="text-3xl font-bold text-gray-800">Reporte 607 - Ventas</h1>
            <p class="text-gray-600 mt-1">Informe de Ventas según normativa DGII</p>
          </div>
          <div class="flex gap-2">
            <Button
              label="Nueva Venta"
              icon="pi pi-plus"
              severity="primary"
              @click="router.push('/vender')"
            />
            <Button
              label="Ver Todas"
              icon="pi pi-list"
              severity="secondary"
              @click="cargarTodasLasVentas"
              :loading="loading"
            />
            <Button
              label="Excel"
              icon="pi pi-file-excel"
              severity="success"
              @click="exportarExcel"
              :disabled="loading || ventasFiltradas.length === 0"
            />
            <Button
              label="TXT DGII"
              icon="pi pi-file"
              severity="info"
              @click="exportarTXT"
              :disabled="loading || ventasFiltradas.length === 0"
            />
            <Button
              label="PDF"
              icon="pi pi-file-pdf"
              severity="danger"
              @click="exportarPDF"
              :disabled="loading || ventasFiltradas.length === 0"
            />
          </div>
        </div>
      </div>

      <!-- Filtros -->
      <Card class="mb-6 shadow-lg">
        <template #title>
          <div class="flex items-center gap-2">
            <i class="pi pi-filter text-green-600"></i>
            <span>Filtros y Búsqueda</span>
          </div>
        </template>
        <template #content>
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="flex flex-col gap-2">
              <label class="font-semibold text-sm text-gray-700">Rango de Fechas</label>
              <DatePicker
                v-model="rangoFecha"
                selectionMode="range"
                dateFormat="dd/mm/yy"
                :showIcon="true"
                placeholder="Selecciona rango"
                class="w-full"
              />
            </div>
            <div class="flex flex-col gap-2">
              <label class="font-semibold text-sm text-gray-700">Tipo de Comprobante</label>
              <Dropdown
                v-model="tipoComprobanteFilter"
                :options="tiposComprobantes"
                optionLabel="label"
                optionValue="value"
                placeholder="Todos los tipos"
                class="w-full"
              />
            </div>
            <div class="flex flex-col gap-2">
              <label class="font-semibold text-sm text-gray-700">Búsqueda</label>
              <InputText
                v-model="busqueda"
                placeholder="Cliente, NCF, Factura..."
                class="w-full"
              />
            </div>
            <div class="flex items-end">
              <Button
                label="Cargar Datos"
                icon="pi pi-search"
                severity="primary"
                @click="cargarDatos"
                :loading="loading"
                class="w-full"
              />
            </div>
          </div>
        </template>
      </Card>

      <!-- Estadísticas -->
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card class="shadow-lg">
          <template #content>
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm mb-1">Total Ventas</p>
                <p class="text-2xl font-bold text-green-600">{{ totalVentas }}</p>
              </div>
              <div class="bg-green-100 p-3 rounded-full">
                <i class="pi pi-shopping-bag text-green-600 text-2xl"></i>
              </div>
            </div>
          </template>
        </Card>

        <Card class="shadow-lg">
          <template #content>
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm mb-1">Monto Total</p>
                <p class="text-2xl font-bold text-blue-600">
                  RD$ {{ totalMonto.toLocaleString('es-DO', { minimumFractionDigits: 2 }) }}
                </p>
              </div>
              <div class="bg-blue-100 p-3 rounded-full">
                <i class="pi pi-dollar text-blue-600 text-2xl"></i>
              </div>
            </div>
          </template>
        </Card>

        <Card class="shadow-lg">
          <template #content>
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm mb-1">ITBIS Total</p>
                <p class="text-2xl font-bold text-orange-600">
                  RD$ {{ totalITBIS.toLocaleString('es-DO', { minimumFractionDigits: 2 }) }}
                </p>
              </div>
              <div class="bg-orange-100 p-3 rounded-full">
                <i class="pi pi-percentage text-orange-600 text-2xl"></i>
              </div>
            </div>
          </template>
        </Card>

        <Card class="shadow-lg">
          <template #content>
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm mb-1">Base Imponible</p>
                <p class="text-2xl font-bold text-purple-600">
                  RD$ {{ totalBase.toLocaleString('es-DO', { minimumFractionDigits: 2 }) }}
                </p>
              </div>
              <div class="bg-purple-100 p-3 rounded-full">
                <i class="pi pi-calculator text-purple-600 text-2xl"></i>
              </div>
            </div>
          </template>
        </Card>
      </div>

      <!-- Tabla de Datos -->
      <Card class="shadow-lg">
        <template #title>
          <div class="flex items-center gap-2">
            <i class="pi pi-list text-green-600"></i>
            <span>Detalle de Ventas</span>
          </div>
        </template>
        <template #content>
          <DataTable
            :value="ventasFiltradas"
            :paginator="true"
            :rows="20"
            :loading="loading"
            stripedRows
            showGridlines
            responsiveLayout="scroll"
            paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
            :rowsPerPageOptions="[10, 20, 50, 100]"
            currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords} ventas"
          >
            <template #empty>
              <div class="text-center py-8">
                <i class="pi pi-inbox text-gray-400 text-5xl mb-3"></i>
                <p class="text-gray-500">No se encontraron ventas en el periodo seleccionado</p>
              </div>
            </template>

            <Column field="linea" header="#" :style="{ width: '60px' }"></Column>
            <Column field="fecha_emision" header="Fecha" :style="{ minWidth: '100px' }">
              <template #body="slotProps">
                {{ formatearFecha(slotProps.data.fecha_emision) }}
              </template>
            </Column>
            <Column field="rnc_cedula" header="RNC/Cédula" :style="{ minWidth: '120px' }"></Column>
            <Column field="nombre_cliente" header="Cliente" :style="{ minWidth: '200px' }"></Column>
            <Column field="comprobante" header="Tipo Comprobante" :style="{ minWidth: '180px' }">
              <template #body="slotProps">
                <Tag :value="slotProps.data.comprobante || 'Sin Comprobante'" :severity="getSeverityComprobante(slotProps.data.comprobante)" />
              </template>
            </Column>
            <Column field="tipo_factura" header="NCF" :style="{ minWidth: '150px' }"></Column>
            <Column field="no_factura" header="No. Factura" :style="{ minWidth: '120px' }"></Column>
            <Column field="subtotal" header="Base" :style="{ minWidth: '120px' }">
              <template #body="slotProps">
                <span class="font-semibold text-blue-700">
                  RD$ {{ parseFloat(slotProps.data.subtotal || 0).toLocaleString('es-DO', { minimumFractionDigits: 2 }) }}
                </span>
              </template>
            </Column>
            <Column field="impuesto" header="ITBIS" :style="{ minWidth: '120px' }">
              <template #body="slotProps">
                <span class="font-semibold text-orange-700">
                  RD$ {{ parseFloat(slotProps.data.impuesto || 0).toLocaleString('es-DO', { minimumFractionDigits: 2 }) }}
                </span>
              </template>
            </Column>
            <Column field="total" header="Total" :style="{ minWidth: '120px' }">
              <template #body="slotProps">
                <span class="font-bold text-green-700">
                  RD$ {{ parseFloat(slotProps.data.total || 0).toLocaleString('es-DO', { minimumFractionDigits: 2 }) }}
                </span>
              </template>
            </Column>
          </DataTable>
        </template>
      </Card>

      <Toast />
    </div>
  </main>
</template>

<script>
const getSeverityComprobante = (comprobante) => {
  const severities = {
    'CONSUMIDOR FINAL': 'info',
    'COMPROBANTE FISCAL': 'success',
    'REGIMEN ESPECIAL': 'warning',
    'COMPROBANTE GUBERNAMENTAL': 'secondary',
    'SIN COMPROBANTE': 'danger'
  };
  return severities[comprobante] || 'secondary';
};
</script>

<style scoped>
.content-wrapper {
  min-height: calc(100vh - 60px);
}
</style>
