...
describe('Caja: número de orden en el ticket', () => {
  ...
  it('mantiene las últimas 100 facturas y permite filtrar solo las del turno', () => {
    ...
    const indiceComparacionPendientes = cajaSource.indexOf('const existingIds = new Set(')
    const indiceAsignacionPendientes = cajaSource.indexOf('facturasSinCobrar.value = facturasDelAlmacen')
    expect(indiceComparacionPendientes).toBeGreaterThan(-1)
    expect(indiceAsignacionPendientes).toBeGreaterThan(indiceComparacionPendientes)
    expect(cajaSource).toContain('facturasPendientesInicializadas.value && newFacturas.length > 0')
    ...
    expect(cajaSource).toContain('if (consultandoFacturasPendientes) return;')
  ...
  it('reemplaza el selector SweetAlert sin perder las rutas de impresión', () => {
    expect(cajaSource).toContain('label="Ver productos"')
    expect(cajaSource).toContain('@click="fnProductos(campoFactura)"')
    expect(cajaSource).toContain("Productos - Factura ${escaparHtml(factura?.no_factura || '')}")
    expect(cajaSource).toContain('<th style="padding:10px 8px;text-align:center;">Cant.</th>')
    expect(cajaSource).toContain("container: 'caja-productos-swal'")
    expect(cajaSource).toContain(':global(.caja-productos-swal)')
    ...
    expect((cajaSource.match(/await finalizarCobroExitoso\(datosFactura\)/g) || []).length).toBe(3)
    expect(cajaSource).toContain('La selección de impresión debe mostrarse inmediatamente después del cobro.')
    expect(cajaSource).toContain('La actualización de la pantalla no debe bloquear la selección de impresión.')
  })
})
