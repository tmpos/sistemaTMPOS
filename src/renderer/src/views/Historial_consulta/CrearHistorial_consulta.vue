...
/************************************************************************/
const datoscamposHistorial_consulta = ref({})
const codigoUnico = ref(generarCodigoUnico());
const fecha = ref(nfecha('fecha'));
const position = "top";
/************************************************************************/
const fetchAndSetupData = async () => {
    const jsonData = await arrayToObjetoFromTablaOffline('historial_consulta');
    datoscamposHistorial_consulta.value = jsonData;
};
/************************************************************************/
onMounted(async() => {
...
async function enviarDatos(event) {
  ...
  const url = link.value+api.value+"/insertar/historial_consulta";
  if (!datoscamposHistorial_consulta.value) {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Datos incompletos, no se puede Enviar.', life: 3000 });
    return;
  }
  if (datoscamposHistorial_consulta.value.hasOwnProperty('created_at')) {
     datoscamposHistorial_consulta.value.created_at = nfecha('timestamp')
     datoscamposHistorial_consulta.value.updated_at = nfecha('timestamp')
    }
  const envioDatos = await peticionesFetchOffline('insertData', 'historial_consulta', JSON.stringify(datoscamposHistorial_consulta.value));
  if (envioDatos[0] == 'ok') {
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    router.push({ path: `/historial_consulta` });
  }
})
  }else{
...
<div class="col-span-12 sm:col-span-12 md:col-span-6 lg:col-span-6 xl:col-span-6 2xl:col-span-6">
                    ...
                    <InputText type="text" class="form-input w-full "  v-solonumeros v-model="datoscamposHistorial_consulta.imei" placeholder="imei" name="crearimei" id="imei" />
                </div>
<div class="col-span-12 sm:col-span-12 md:col-span-6 lg:col-span-3 xl:col-span-3 2xl:col-span-3">
                   ...
                   <DatePicker  v-model="datoscamposHistorial_consulta.fecha" showButtonBar fluid />
            </div>
<div class="col-span-12 sm:col-span-12 md:col-span-6 lg:col-span-3 xl:col-span-3 2xl:col-span-3">
                   <label for="hora" class="block text-sm font-medium text-gray-700 dark:text-gray-400">Hora</label>
                   <DatePicker  v-model="datoscamposHistorial_consulta.hora" timeOnly fluid />
            </div>
<div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12 2xl:col-span-12">
                   ...
                   <Textarea id="creardatos_recibidos" rows="3" class="form-textarea w-full "  v-model="datoscamposHistorial_consulta.datos_recibidos" placeholder="Datos_recibidos"></textarea>
                </div>

<div class="form-group col-span-6" hidden>
<label for="created_atAgregarDatos">CREATED_AT</label>
<input type="input" v-model="datoscamposHistorial_consulta.created_at" name="created_at"  class="form-control " id="created_atAgregarDatos"  placeholder="created_at" maxlength="">
</div>
<div class="form-group col-span-6" hidden>
<label for="updated_atAgregarDatos">UPDATED_AT</label>
<input type="input" v-model="datoscamposHistorial_consulta.updated_at" name="updated_at"  class="form-control " id="updated_atAgregarDatos"  placeholder="updated_at" maxlength="">
</div>

<div class="form-group col-span-12" hidden>
<label for="usuarioAgregarDatos">USUARIO</label>
<input type="input" v-model="datoscamposHistorial_consulta.usuario" name="usuario"  class="form-control " id="usuarioAgregarDatos"  placeholder="usuario" maxlength="250">
</div>
