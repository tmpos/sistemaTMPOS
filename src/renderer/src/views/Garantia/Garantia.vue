...
/************************************************************************/
const camposArray = ["referencia","garantia","usuario"];
/************************************************************************/
...
const GarantiaEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposGarantia.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
async function campos() {
  ...
  datoscamposGarantia.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExisteOffline('garantia', camposArray,toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'garantia');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/garantia";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'garantia',JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
    visible.value = false;
    fetchAndSetupData();
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/garantia";
  if (datoscamposGarantia.value.hasOwnProperty('created_at')) {
    datoscamposGarantia.value.created_at = nfecha('timestamp');
    datoscamposGarantia.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await peticionesFetchOffline('insertData', 'garantia',JSON.stringify(datoscamposGarantia.value));
  if (envioDatos[0] == 'ok') {
    fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await peticionesFetchOffline('deleteEntry', 'garantia',id);
                            } catch (error) {
...
/************************************************************************/
const itemsGarantia = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleGarantia = (event, rowData) => {
...
/************************************************************************/
const filteredGarantia = computed(() => {
...
<div class="grid grid-cols-12 gap-4">
      <div class="md:col-span-12">
        <fieldset class="border p-3 rounded mb-2">
          <legend class="float-none w-auto px-2">Datos de Garantia</legend>
            <div class="grid grid-cols-12 gap-4">
...
<a href="#" v-if="usuarioLocal.usuario =='Soporte'"  @click="borrarTodo" class="btn btn-danger btnaccion cartelito float-end ms-1" data-toggle="tooltip" title="Borrar Todo" id="borrartodo"><i class="icon-trash"></i> Borrar Todo</a>
 ...
 <input v-model="searchQuery" placeholder="Buscar garantia..." class="p-inputtext p-component" style="margin-bottom: 10px;" />
</div>
<DataTable 
    :value="filteredGarantia"  
    scrollable 
    scrollHeight="600px"  
    dataKey="id" 
    paginator 
    ...
    <Column header="Options">
        <template #body="slotProps">
            <Button 
                icon="pi pi-cog" 
                @click="toggleGarantia($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
            />
            <Menu 
                ref="menu" 
                id="overlay_menu_Garantia" 
                :model="itemsGarantia" 
                :popup="true"
            />
        </template>
...
<Column field="garantia" header="Garantia"></Column>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Garantia" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
  ...
  <fieldset class="border p-3 rounded mb-2">
    <legend class="float-none w-auto px-2">Garantia</legend>
     <form id="formularioActualizarGarantia" action="" method="">
...
<label for="referencia-Actualizador">REFERENCIA</label>
<input type="input" v-model="datoscampos.referencia" name="referencia"  class="form-control" id="referencia-Actualizador"  placeholder="referencia" v-mayuscula maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="garantia-Actualizador">GARANTIA</label>
<textarea class="form-control " id="garantia-Actualizador" name="garantia" v-model="datoscampos.garantia" cols="30" rows="3" ></textarea>
</div>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Garantia" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="inline-flex align-items-center justify-content-center gap-2">
  ...
  <fieldset class="border p-3 rounded mb-2">
    <legend class="float-none w-auto px-2">Garantia</legend>
     <form id="formularioActualizarGarantia" action="" method="">
         <div  style="margin-top: 15px;color: #34AAB2;" class="grid grid-cols-12 gap-4">
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="referenciaAgregarDatos">REFERENCIA</label>
<input type="input" v-model="datoscamposGarantia.referencia" name="referencia"  class="form-control mayusc" id="referenciaAgregarDatos" v-mayuscula placeholder="referencia" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="garantiaAgregarDatos">GARANTIA</label>
<textarea class="form-control " v-model="datoscamposGarantia.garantia" id="garantiaAgregarDatos" name="garantia" cols="30" rows="3" ></textarea>
</div>
<div class="form-group col-span-6" hidden>
<label for="created_atAgregarDatos">CREATED_AT</label>
<input type="input" v-model="datoscamposGarantia.created_at" name="created_at"  class="form-control " id="created_atAgregarDatos"  placeholder="created_at" maxlength="">
</div>
<div class="form-group col-span-6" hidden>
<label for="updated_atAgregarDatos">UPDATED_AT</label>
<input type="input" v-model="datoscamposGarantia.updated_at" name="updated_at"  class="form-control " id="updated_atAgregarDatos"  placeholder="updated_at" maxlength="">
</div>
<div class="form-group col-span-12" hidden>
<label for="usuarioAgregarDatos">USUARIO</label>
<input type="input" v-model="datoscamposGarantia.usuario" name="usuario"  class="form-control " id="usuarioAgregarDatos"  placeholder="usuario" maxlength="250">
</div>
