...
import { peticionesFetch, nfecha, envioElectron, enviarDatosLocalStorage, encryptarPassword, peticionesFetchOffline, arrayToObjetoFromTablaOffline, crearTablaSiNoExisteOffline } from '../../funciones/funciones.js';
...
const fetchData = async () => {
    const fechasSemana = nfecha('ultimos7dias');
...
const response = await peticionesFetchOffline('datosVentasPorRango', fechasSemana.fechainicio,fechasSemana.fechafin);
...
const getUltimos7Dias = () => {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const diaFormateado = dia.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'short' });
...
/************************************************************************/
const procesarDatosUltimos7Dias = (datos) => {
    const resultadosPorSemana = {
        ...
        dias: getUltimos7Dias(), // Obtener los últimos 7 días
    };

    // Procesar facturas y otros datos para los últimos 7 días
    if (datos.facturas && datos.facturas.length > 0) {
        datos.facturas.forEach((factura) => {
            const fecha = new Date(factura.updated_at);
            const diferenciaDias = Math.floor((new Date() - fecha) / (1000 * 60 * 60 * 24));
            if (diferenciaDias < 7) {
                const diaIndex = 6 - diferenciaDias; // Ajustar para que los datos coincidan con las etiquetas
                resultadosPorSemana.ventas[diaIndex] += parseFloat(factura.total) || 0;
                resultadosPorSemana.ganancias[diaIndex] += parseFloat(factura.ganancia) || 0;
                resultadosPorSemana.impuestos[diaIndex] += parseFloat(factura.impuesto) || 0;
            }
        });
    }

    // Procesar gastos y otros datos similares para los últimos 7 días
    if (datos.gastos && datos.gastos.length > 0) {
        datos.gastos.forEach((gastos) => {
            const fecha = new Date(gastos.updated_at);
            const diferenciaDias = Math.floor((new Date() - fecha) / (1000 * 60 * 60 * 24));
            if (diferenciaDias < 7) {
                const diaIndex = 6 - diferenciaDias;
                resultadosPorSemana.gastos[diaIndex] += parseFloat(gastos.cantidad) || 0;
            }
        });
    }

    // Repetir este mismo patrón para devoluciones, abonos, etc.
...
onMounted(async () => {
    ...
    const resultados = procesarDatosUltimos7Dias(dataAarray.value);
    if (resultados) {
...
const fnDownloadFacturas = ()=>{
        const data = facturasArray.value.map(item => ({
            fecha: item.fecha_emision,
            cliente: item.nombre_cliente,
            tipo_factura: item.tipo_factura,
            rnc_cliente: item.nombre_cliente,
...
function calcularTotalesPorCategoria(datos) {
  ...
  if (datos.ventas && Array.isArray(datos.ventas)) {
    totales.ventas = datos.ventas.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.ganancias && Array.isArray(datos.ganancias)) {
    totales.ganancias = datos.ganancias.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.impuestos && Array.isArray(datos.impuestos)) {
    totales.impuestos = datos.impuestos.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.gastos && Array.isArray(datos.gastos)) {
    totales.gastos = datos.gastos.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.devoluciones && Array.isArray(datos.devoluciones)) {
    totales.devoluciones = datos.devoluciones.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.abonos && Array.isArray(datos.abonos)) {
    totales.abonos = datos.abonos.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.taller && Array.isArray(datos.taller)) {
    totales.taller = datos.taller.reduce((acc, valor) => acc + valor, 0);
  }

  if (datos.cuentas_cobrar && Array.isArray(datos.cuentas_cobrar)) {
    totales.cuentas_cobrar = datos.cuentas_cobrar.reduce((acc, valor) => acc + valor, 0);
  }
...
const fnPrintFacturas = ()=>{
   const totales = calcularTotalesPorCategoria(resultadosPorSemanaVista.value);
   const datosEmpresa = JSON.stringify(enviarDatosLocalStorage())
        ...
        <div class="md:col-span-12 mt-1">
          <div class="card shadow-sm">
            <div class="card-header bg-primary text-white text-center">
              <h5>Resumen Financiero de los Ultimos 7 días</h5>
            </div>
            <div class="card-body">
              <div class="grid grid-cols-12 gap-4 text-center">
                ...
                <div class="md:col-span-4 lg:col-span-3 mb-3" v-if="resultadosPorSemanaVista">
