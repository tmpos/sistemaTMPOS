...
/************************************************************************/
const camposArray = ["mensajeria","mensaje","usuario"];
/************************************************************************/
...
/************************************************************************/
const datoscamposMensajeria = ref({})
/************************************************************************/
...
const MensajeriaEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposMensajeria.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
const response = await peticionesFetchOffline('getDataAsArray', 'mensajeria');
    const jsonData = response;
    data.value = jsonData;
};
/************************************************************************/
async function campos() {
  const campos = await arrayToObjetoFromTablaOffline('mensajeria');
  datoscamposMensajeria.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExisteOffline('mensajeria', camposArray,toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'mensaria');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/mensajeria";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'mensajeria',JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
    visible.value = false;
    fetchAndSetupData();
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/mensajeria";
  if (datoscamposMensajeria.value.hasOwnProperty('created_at')) {
    datoscamposMensajeria.value.created_at = nfecha('timestamp');
    datoscamposMensajeria.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await peticionesFetchOffline('insertData', 'mensajeria',JSON.stringify(datoscamposMensajeria.value));
  if (envioDatos[0] == 'ok') {
    fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await peticionesFetchOffline('deleteEntry', 'mensajeria',id);
                            } catch (error) {
...
const toggleMensajeria = (event, rowData) => {
currentRowData.value = rowData;
itemsMensajeria.value = [
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry', 'mensajeria',rowData.id);
...
/************************************************************************/
const filteredMensajeria = computed(() => {
...
<div class="grid grid-cols-12 gap-4">
      <div class="md:col-span-12">
        <fieldset class="border p-3 rounded mb-2">
          <legend class="float-none w-auto px-2">Datos de Mensajeria</legend>
            <div class="grid grid-cols-12 gap-4">
...
<a href="#" v-if="usuarioLocal.usuario =='Soporte'"  @click="borrarTodo" class="btn btn-danger btnaccion cartelito float-end ms-1" data-toggle="tooltip" title="Borrar Todo" id="borrartodo"><i class="icon-trash"></i> Borrar Todo</a>
 ...
 <input v-model="searchQuery" placeholder="Buscar mensajeria..." class="p-inputtext p-component" style="margin-bottom: 10px;" />
</div>
<DataTable 
    :value="filteredMensajeria"  
    scrollable 
    scrollHeight="600px"  
    dataKey="id" 
    paginator 
    ...
    <Column header="Options">
        <template #body="slotProps">
            <Button 
                icon="pi pi-cog" 
                @click="toggleMensajeria($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
            />
            <Menu 
                ref="menu" 
                id="overlay_menu_Mensajeria" 
                :model="itemsMensajeria" 
                :popup="true"
            />
        </template>
    </Column>
    <Column field="mensajeria" header="Mensajeria"></Column>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Mensajeria" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
  ...
  <fieldset class="border p-3 rounded mb-2">
    <legend class="float-none w-auto px-2">Mensajeria</legend>
     <form id="formularioActualizarMensajeria" action="" method="">
...
<label for="mensajeria-Actualizador">MENSAJERIA</label>
<input type="input" v-model="datoscampos.mensajeria" name="mensajeria"  class="form-control" id="mensajeria-Actualizador"  placeholder="mensajeria" v-mayuscula maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Mensajeria" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="inline-flex align-items-center justify-content-center gap-2">
  ...
  <fieldset class="border p-3 rounded mb-2">
    <legend class="float-none w-auto px-2">Mensajeria</legend>
     <form id="formularioActualizarMensajeria" action="" method="">
         <div  style="margin-top: 15px;color: #34AAB2;" class="grid grid-cols-12 gap-4">
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="mensajeriaAgregarDatos">MENSAJERIA</label>
<input type="input" v-model="datoscamposMensajeria.mensajeria" name="mensajeria"  class="form-control mayusc" id="mensajeriaAgregarDatos" v-mayuscula placeholder="mensajeria" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="mensajeAgregarDatos">MENSAJE</label>
<textarea class="form-control " v-model="datoscamposMensajeria.mensaje" id="mensajeAgregarDatos" name="mensaje" cols="30" rows="3" ></textarea>
</div>
<div class="form-group col-span-6" hidden>
<label for="created_atAgregarDatos">CREATED_AT</label>
<input type="input" v-model="datoscamposMensajeria.created_at" name="created_at"  class="form-control " id="created_atAgregarDatos"  placeholder="created_at" maxlength="">
</div>
<div class="form-group col-span-6" hidden>
<label for="updated_atAgregarDatos">UPDATED_AT</label>
<input type="input" v-model="datoscamposMensajeria.updated_at" name="updated_at"  class="form-control " id="updated_atAgregarDatos"  placeholder="updated_at" maxlength="">
</div>
<div class="form-group col-span-12" hidden>
<label for="usuarioAgregarDatos">USUARIO</label>
<input type="input" v-model="datoscamposMensajeria.usuario" name="usuario"  class="form-control " id="usuarioAgregarDatos"  placeholder="usuario" maxlength="250">
</div>
