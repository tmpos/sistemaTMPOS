...
/************************************************************************/
const camposArray = ['nombre', 'codigo'];
/************************************************************************/
...
const RutasEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposRutas.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
async function campos() {
  const campos = await arrayToObjetoFromTablaOffline('rutas');
  datoscamposRutas.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExisteOffline('rutas', camposArray, toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'rutas');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/rutas";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'rutas',JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
    visible.value = false;
    fetchAndSetupData();
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/rutas";
  if (datoscamposRutas.value.hasOwnProperty('created_at')) {
    datoscamposRutas.value.created_at = nfecha('timestamp');
    datoscamposRutas.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await peticionesFetchOffline('insertData', 'rutas',JSON.stringify(datoscamposRutas.value));
  if (envioDatos[0] == 'ok') {
    fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await peticionesFetchOffline('deleteEntry', 'rutas',id);
                            } catch (error) {
...
/************************************************************************/
const itemsRutas = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleRutas = (event, rowData) => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry', 'rutas',rowData.id);
...
<fieldset class="border p-3 rounded-md mb-2">
  <legend class="px-2">Datos de Rutas</legend>
    ...
    <div class="ml-auto">
      ...
      <div class="w-full">
        <div class="flex justify-end mb-4">
          <InputText v-model="searchQuery" placeholder="Buscar rutas..." class="p-inputtext p-component" />
        </div>
        <DataTable
          :value="filteredRutas"
          scrollable
         ...
         <Column selectionMode="multiple" headerStyle="width: 3rem"></Column>
          <Column header="Options">
            <template #body="slotProps">
              <Button
                icon="pi pi-cog"
                @click="toggleRutas($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
              />
              <Menu
                ref="menu"
                id="overlay_menu_Rutas"
                :model="itemsRutas"
                :popup="true"
              />
            </template>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Rutas" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
  <template #header>
  ...
  <fieldset class="border p-3 rounded-md mb-2">
    <legend class="px-2">Rutas</legend>
    <form id="formularioActualizarRutas" action="" method="">
...
<div class="col-span-12 sm:col-span-12 md:col-span-4 lg:col-span-4 xl:col-span-4 2xl:col-span-4">
                ...
                <InputText type="text"  v-solonumeros class="form-input w-full " v-model="datoscampos.codigo" name="codigo" placeholder="codigo" id="actualizarcodigo" />
            </div>
        <div class="hidden col-span-12">
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Rutas" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
  <template #header>
    <div class="inline-flex align-items-center justify-content-center gap-2">
  ...
  <fieldset class="border p-3 rounded-md mb-2">
    <legend class="px-2">Rutas</legend>
    <form id="formularioCrearRutas" action="" method="">
      <div class="grid grid-cols-12 gap-4 mt-4 text-blue-600">

<div class="col-span-12 sm:col-span-12 md:col-span-8 lg:col-span-8 xl:col-span-8 2xl:col-span-8">
                    <label for="nombre" class="block text-sm font-medium text-gray-700 dark:text-gray-400">NOMBRE</label>
                    <InputText type="text" class="form-input w-full "  v-mayuscula v-model="datoscamposRutas.nombre" placeholder="nombre" name="crearnombre" id="nombre" />
                </div>
<div class="col-span-12 sm:col-span-12 md:col-span-4 lg:col-span-4 xl:col-span-4 2xl:col-span-4">
                    <label for="codigo" class="block text-sm font-medium text-gray-700 dark:text-gray-400">CODIGO</label>
                    <InputText type="text" class="form-input w-full "  v-solonumeros v-model="datoscamposRutas.codigo" placeholder="codigo" name="crearcodigo" id="codigo" />
                </div>
        ...
        <div class="hidden col-span-6">
          ...
          <InputText v-model="datoscamposRutas.created_at" name="created_at" id="created_atAgregarDatos" placeholder="created_at" class="w-full" />
        </div>
        <div class="hidden col-span-6">
          <label for="updated_atAgregarDatos" class="block text-sm font-medium text-gray-700">UPDATED_AT</label>
          <InputText v-model="datoscamposRutas.updated_at" name="updated_at" id="updated_atAgregarDatos" placeholder="updated_at" class="w-full" />
        </div>
        <div class="hidden col-span-12">
          <label for="usuarioAgregarDatos" class="block text-sm font-medium text-gray-700">USUARIO</label>
          <InputText v-model="datoscamposRutas.usuario" name="usuario" id="usuarioAgregarDatos" placeholder="usuario" maxlength="250" class="w-full" />
        </div>
      </div>
    </form>
  </fieldset>
