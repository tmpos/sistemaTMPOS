...
/************************************************************************/
const camposArray = ['fecha', 'hora', 'total_celulares', 'total_inventario', 'celulares_iventariados','total_celulares_vendidos'];
/************************************************************************/
...
const Inventario_celularEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposInventario_celular.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
/*if(navigator.onLine){*/
   //await crearTablaSiNoExiste(link.value, api.value, 'inventario_celular', camposArray, tokenCifrado.value,toast);
/*}else{
  if(window.electron){
  }   
}*/
     await crearTablaSiNoExisteOffline('inventario_celular',camposArray.join(','),toast)
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'inventario_celular');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                            const envioDatos = await peticionesFetchOffline('deleteEntry','inventario_celular', id);
                            } catch (error) {
...
/************************************************************************/
const itemsInventario_celular = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleInventario_celular = (event, rowData) => {
currentRowData.value = rowData;
itemsInventario_celular.value = [
{ label: 'Editar', icon: 'pi pi-pencil', command: () => { 
router.push({ path: `/editarinventario_celular/${currentRowData.value.id}` });
} },
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry','inventario_celular', rowData.id);
...
/************************************************************************/
const filteredInventario_celular = computed(() => {
...
const onRowSelect = (event) => {
 router.push({ path: `/editarinventario_celular/${event.data.id}` });
...
const fnPrint = async () => {
  try {
    ...
    let celulares = [];
    try {
      celulares = JSON.parse(currentRowData.value.celulares_iventariados || "[]");
    } catch (e) {
    ...
    const totalCelulares = parseInt(currentRowData.value.total_celulares) || celulares.length || 0;
 ...
 <Fieldset legend="Datos de Inventario_celular">
  <div class="flex items-center">
    <div class="flex space-x-2">
      ...
      <Button as="router-link" icon="pi pi-plus-circle" to="/crearinventario_celular" class="ms-1" />
    ...
    <div class="ml-auto">
      ...
      <div class="w-full">
        <div class="flex justify-end mb-4">
          <InputText v-model="searchQuery" placeholder="Buscar inventario_celular..." class="p-inputtext p-component" />
        </div>
        <DataTable
          :value="filteredInventario_celular"
          scrollable
          scrollHeight="600px"
          dataKey="id"
          paginator
        ...
        <template #body="{ data }">
          ...
          <Column header="Options">
            <template #body="slotProps">
              <Button
                icon="pi pi-cog"
                @click="toggleInventario_celular($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
              />
              <Menu
                ref="menu"
                id="overlay_menu_Inventario_celular"
                :model="itemsInventario_celular"
                :popup="true"
              />
            </template>
          </Column>
...
<Column field="total_celulares" header="Total_celulares"></Column>
<Column field="total_inventario" header="Total_inventario"></Column>
<!-- <Column field="celulares_iventariados" header="Celulares_iventariados"></Column> -->
        </DataTable>
      </div>
    </div>
      </template>
