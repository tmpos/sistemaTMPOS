...
import { nfecha, peticionesFetchOffline, crearTablaSiNoExisteOffline } from '@/funciones/funciones.js';
...
// Lista de productos en uso interno
...
const productosFiltrados = computed(() => {
  ...
  let resultado = [...productosUsoInterno.value];
...
const estadisticas = computed(() => {
  const productosFiltradosPorFecha = productosUsoInterno.value.filter(p =>
    compararSoloFecha(p.created_at)
  );

  const total = productosFiltradosPorFecha.length;
  const enUso = productosFiltradosPorFecha.filter(p => p.estado === 'En uso').length;
  const devueltos = productosFiltradosPorFecha.filter(p => p.estado === 'Devuelto a inventario').length;
  const consumidos = productosFiltradosPorFecha.filter(p => p.estado === 'Consumido').length;
    ...
    if (producto) {
      ...
      if (producto.categoria === 'CELULARES' || producto.categoria === 'ELECTRODOMESTICOS') {
        ...
        toast.add({
          ...
          detail: `Se encontraron ${imeiDisponibles.value.length} unidades disponibles. Selecciona cuántas deseas retirar.`,
          life: 5000
        });
      } else {
  ...
  try {
    if (tieneSeriales.value) {
      ...
      toast.add({
        severity: 'success',
        summary: 'Éxito',
        detail: `${imeiSeleccionados.value.length} producto(s) retirado(s) para uso interno`,
        life: 3000
      });
...
const generarPDF = async () => {
  try {
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Productos de Uso Interno</title>
        <style>
      ...
      <body>
        <div class="header">
          ...
          <h2>Reporte de Productos de Uso Interno</h2>
...
<template>
  <div class="productos-uso-interno-page">
    ...
    <div class="page-header">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h1 class="text-3xl font-bold text-gray-800 m-0">Productos de Uso Interno</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      ...
      <Card class="stat-card">
        <template #content>
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 mb-1">En Uso</p>
        ...
        <div v-if="vistaActual === 'historial'" class="space-y-4">
          ...
          <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
            <div class="flex flex-wrap items-end gap-4">
              ...
              <div class="text-sm text-gray-600 bg-blue-50 px-3 py-2 rounded-lg">
                ...
                <span v-else-if="filtroFecha === 'ultimos7dias'">Mostrando registros de los <strong>últimos 7 días</strong></span>
          ...
          <div class="flex flex-wrap items-center justify-between gap-3">
            <span class="p-input-icon-left flex-1" style="min-width: 300px">
              ...
              <InputText
                v-model="searchQuery"
                placeholder="Buscar por producto, código, motivo, área, estado o IMEI/Serial..."
                class="w-full"
              />
            </span>
          ...
          <DataTable
            ...
            scrollHeight="600px"
            class="mt-4"
            :emptyMessage="'No hay registros de productos de uso interno'"
          >
            ...
            <Column field="cantidad" header="Cant." style="min-width: 80px" class="text-center" sortable>
              <template #body="{ data }">
            ...
            <Column field="motivo_retiro" header="Motivo" style="min-width: 150px" sortable>
              <template #body="{ data }">
                <div class="text-sm">{{ data.motivo_retiro }}</div>
              </template>
            </Column>

            <Column field="area_uso" header="Área de Uso" style="min-width: 150px" sortable>
              <template #body="{ data }">
                <div class="text-sm">{{ data.area_uso }}</div>
              </template>
            </Column>

            <Column field="descripcion" header="Descripción" style="min-width: 200px">
              <template #body="{ data }">
                <div class="text-sm text-gray-600 truncate" :title="data.descripcion">
            ...
            <div v-if="formulario.codigo_producto" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-blue-200">
              ...
              <div v-if="tieneSeriales" class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-3">
                  ¿Cuántos deseas retirar? <span class="text-red-500">*</span>
                </label>
                <SelectButton
                ...
                <div v-else class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <div class="flex items-start gap-2">
                    ...
                    <div>
                      <p class="font-semibold text-yellow-800">Todos los IMEI/Seriales serán retirados</p>
                      <p class="text-sm text-yellow-700 mt-1">
                        Se retirarán <strong>{{ imeiDisponibles.length }}</strong> unidades para uso interno
                      </p>
                    </div>
                  </div>
                </div>
    ...
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500 m-0">Reporte de Productos de Uso Interno</p>
        </div>
      </div>
    </template>
