...
/************************************************************************/
const camposArray = ['token', 'fecha', 'fecha_inicio', 'fecha_fin', 'cantidad_inicio', 'efectivo', 'tarjeta', 'transferencia','ventas','devoluciones','ganancias', 'otro', 'gastos', 'total', 'turno', 'nota','estado'];
/************************************************************************/
...
const CuadresEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposCuadres.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
tokenCifrado.value = await encryptarPassword(token.value, 10);
  ...
  if(window.electron){
     ...
     await crearTablaSiNoExisteOffline('cuadres',camposArray.join(','),toast)
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'cuadres');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                            const envioDatos = await peticionesFetchOffline('deleteEntry','cuadres', id);
                            } catch (error) {
...
/************************************************************************/
const itemsCuadres = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleCuadres = (event, rowData) => {
currentRowData.value = rowData;
itemsCuadres.value = [
{ label: 'Editar', icon: 'pi pi-pencil', command: () => { 
router.push({ path: `/editarcuadres/${currentRowData.value.id}` });
} },
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry','cuadres', rowData.id);
...
/************************************************************************/
const filteredCuadres = computed(() => {
...
const onRowSelect = (event) => {
 router.push({ path: `/editarcuadres/${event.data.id}` });
 ...
 <Fieldset legend="Datos de Cuadres">
  <div class="flex items-center">
    <div class="flex space-x-2">
      ...
      <Button as="router-link" v-if="datosEmpresa.usuario.nivel_seguridad === 'Soporte'" icon="pi pi-plus-circle" to="/crearcuadres" class="ms-1" />

      <Button icon="pi pi-trash" v-if="datosEmpresa.usuario.nivel_seguridad === 'Soporte' || datosEmpresa.usuario.nivel_seguridad === 'Administrador'" severity="danger" @click="borrarSeleccionados" data-toggle="tooltip" title="Borrar Selección" id="borrador" />
    </div>
    <div class="ml-auto">
      <Button
      ...
      <div class="w-full">
        <div class="flex justify-end mb-4">
          <InputText v-model="searchQuery" placeholder="Buscar cuadres..." class="p-inputtext p-component" />
        </div>
        <DataTable
          :value="filteredCuadres"
          scrollable
          scrollHeight="600px"
          dataKey="id"
          paginator
        ...
        <template #body="{ data }">
          ...
          <Column header="Options">
            <template #body="slotProps">
              <Button
                icon="pi pi-cog"
                @click="toggleCuadres($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
              />
              <Menu
                ref="menu"
                id="overlay_menu_Cuadres"
                :model="itemsCuadres"
                :popup="true"
              />
            </template>
...
<Column field="cantidad_inicio" header="Cantidad_inicio"></Column>
