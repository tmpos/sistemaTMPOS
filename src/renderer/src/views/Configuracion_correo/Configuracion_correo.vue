...
/************************************************************************/
const datoscamposConfiguracion_correo = ref({})
/************************************************************************/
...
const Configuracion_correoEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposConfiguracion_correo.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
async function campos() {
  const campos = await arrayToObjetoFromTabla('configuracion_correo');
  datoscamposConfiguracion_correo.value = campos;
}
/************************************************************************/
...
//if(offline){
   await crearTablaSiNoExisteOffline('configuracion_correo',camposArray.join(','),toast)
/*}else{
    if(navigator.onLine){
       await crearTablaSiNoExiste(link.value, api.value, 'configuracion_correo', camposArray, tokenCifrado.value,toast);
    }
}*/
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'configuracion_correo');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/configuracion_correo";
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/configuracion_correo";
  if (datoscamposConfiguracion_correo.value.hasOwnProperty('created_at')) {
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                              const envioDatos = await peticionesFetchOffline('deleteEntry','configuracion_correo', id);
                            } catch (error) {
...
/************************************************************************/
const itemsConfiguracion_correo = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleConfiguracion_correo = (event, rowData) => {
currentRowData.value = rowData;
itemsConfiguracion_correo.value = [
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry','configuracion_correo', rowData.id);
...
/************************************************************************/
const filteredConfiguracion_correo = computed(() => {
...
<div class="w-full">
<Fieldset legend="Datos de Configuracion_correo">                 
    ...
    <div class="ml-auto">
      ...
      <div class="w-full">
        <div class="flex justify-end mb-4">
          <InputText v-model="searchQuery" placeholder="Buscar configuracion_correo..." class="p-inputtext p-component" />
        </div>
        <DataTable
          :value="filteredConfiguracion_correo"
          scrollable
          scrollHeight="600px"
          dataKey="id"
          paginator
         ...
         <Column selectionMode="multiple" headerStyle="width: 3rem"></Column>
          <Column header="Options">
            <template #body="slotProps">
              <Button
                icon="pi pi-cog"
                @click="toggleConfiguracion_correo($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
              />
              <Menu
                ref="menu"
                id="overlay_menu_Configuracion_correo"
                :model="itemsConfiguracion_correo"
                :popup="true"
              />
            </template>
          </Column>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Configuracion_correo" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
  <template #header>
    <div class="inline-flex align-items-center justify-content-center gap-2">
      <span class="font-bold whitespace-nowrap">Modal Editar</span>
    </div>
  </template>
<Fieldset legend="Configuracion_correo">

    <form id="formularioActualizarConfiguracion_correo" action="" method="">
      <div class="grid grid-cols-12 gap-4 mt-4 text-blue-600">
        <div class="hidden col-span-12">
         ...
         <div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12 2xl:col-span-12">
                ...
                <InputText type="text" fluid  class=" " v-model="datoscampos.email" name="email" placeholder="email" id="actualizaremail" />
            </div>
<div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12 2xl:col-span-12">
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Configuracion_correo" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
  <template #header>
    <div class="inline-flex align-items-center justify-content-center gap-2">
...
<Fieldset legend="Configuracion_correo">
    ...
    <form id="formularioCrearConfiguracion_correo" action="" method="">
      <div class="grid grid-cols-12 gap-4 mt-4 text-blue-600">

<div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12 2xl:col-span-12">
                    <label for="email" class="block text-sm font-medium text-gray-700 dark:text-gray-400">EMAIL</label>
                    <InputText type="text" fluid class=" "  v-model="datoscamposConfiguracion_correo.email" placeholder="email" name="crearemail" id="email" />
                </div>
<div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12 2xl:col-span-12">
            <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-400">PASSWORD</label>
        ...
        <div class="hidden col-span-6">
          ...
          <InputText v-model="datoscamposConfiguracion_correo.created_at" name="created_at" id="created_atAgregarDatos" placeholder="created_at" class="w-full" />
        </div>
        <div class="hidden col-span-6">
          <label for="updated_atAgregarDatos" class="block text-sm font-medium text-gray-700">UPDATED_AT</label>
          <InputText v-model="datoscamposConfiguracion_correo.updated_at" name="updated_at" id="updated_atAgregarDatos" placeholder="updated_at" class="w-full" />
        </div>
        <div class="hidden col-span-12">
          <label for="usuarioAgregarDatos" class="block text-sm font-medium text-gray-700">USUARIO</label>
          <InputText v-model="datoscamposConfiguracion_correo.usuario" name="usuario" id="usuarioAgregarDatos" placeholder="usuario" maxlength="250" class="w-full" />
        </div>
      </div>
    </form>
</Fieldset>
