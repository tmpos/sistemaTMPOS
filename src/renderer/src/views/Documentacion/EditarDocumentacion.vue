...
const position = "top";
const todosLosdocumentacion = ref([]);
/************************************************************************/
const fetchAllData = async () => {
    const response = await peticionesFetchOffline('getDataAsArray', 'documentacion');
    todosLosdocumentacion.value = response;
    
    // Buscar el documento por ID
    const documentoEncontrado = response.find(datos => datos.id == route.params.id);
    
    if (documentoEncontrado) {
        // Decodificar el contenido antes de asignarlo
        documentoEncontrado.uso = decodeURIComponent(documentoEncontrado.uso);
        datoscampos.value = documentoEncontrado;
    }

    console.log("Documento cargado:", datoscampos.value);
};

/************************************************************************/
function navigate(action) {
    const currentIndex = todosLosdocumentacion.value.findIndex(notacredito => notacredito.id == route.params.id);
    ...
    switch (action) {
        ...
        case 'anterior':
            ...
            break;
        case 'siguiente':
            newIndex = currentIndex + 1 < todosLosdocumentacion.value.length ? currentIndex + 1 : currentIndex;
            break;
        case 'ultimo':
            newIndex = todosLosdocumentacion.value.length - 1;
            break;
        default:
            return;
    }
    datoscampos.value = todosLosdocumentacion.value[newIndex];
    router.push({ path: `/editardocumentacion/${todosLosdocumentacion.value[newIndex].id}` });
}
/************************************************************************/
onMounted(async() => {
...
async function funcionActualizar(e) {
  e.preventDefault();
  const url = link.value+api.value+"/actualizarcampos/documentacion";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'documentacion', JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
...
<Button as="router-link" class="ms-1" icon="pi pi-plus-circle" to="/creardocumentacion" />
...
<div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12 2xl:col-span-12">
  ...
  <!--                  <Textarea id="actualizaruso"  v-model="datoscampos.uso" name="uso" rows="3" class="form-textarea w-full " placeholder="Enter Uso"></textarea> -->

<QuillEditor 
  v-model="datoscampos.uso"
  theme="snow"
  toolbar="minimal"
