...
<main class="min-h-screen bg-gray-50 dark:bg-gray-900">
  ...
  <div class="container mx-auto px-6 py-6">
    ...
    <div v-else class=" dark:bg-gray-800 rounded-xl shadow-lg p-6">
      <form @submit.prevent="funcionActualizar" id="formularioActualizarOrdenes">
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-12 gap-4">
            ...
            <!-- Materiales -->
            <div class="col-span-12">
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2" for="actualizarmateriales">
                <i class="pi pi-align-left text-indigo-600 mr-1"></i>
                Materiales
              </label>
              <Textarea
                fluid
                id="actualizarmateriales"
...
async function fetchAllData() {
  try {
    ...
    todosOrdeness.value = response.reverse()
  } catch (error) {
...
async function navigate(action) {
  const currentIndex = todosOrdeness.value.findIndex((item) => item.id == route.params.id)
  ...
  switch (action) {
    ...
    case 'anterior':
      ...
      break
    case 'siguiente':
      newIndex = currentIndex + 1 < todosOrdeness.value.length ? currentIndex + 1 : currentIndex
      break
    case 'ultimo':
      newIndex = todosOrdeness.value.length - 1
      break
  }

  if (newIndex !== currentIndex) {
    Object.assign(datoscampos, todosOrdeness.value[newIndex])
    router.push({ path: `/editarOrdenes/${todosOrdeness.value[newIndex].id}` })
  }
}
...
async function funcionActualizar(e) {
  ...
  try {
    ...
    const envioDatos = await peticionesFetchOffline('updateData', 'ordenes', JSON.stringify(datosEnviar))
