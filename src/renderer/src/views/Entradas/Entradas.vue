...
/************************************************************************/
const camposArray = ["cajero","cantidad","fecha","hora","turno","descripcion","mes","year","almacen","usuario"];
/************************************************************************/
...
const EntradasEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposEntradas.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
const response = await peticionesFetchOffline('getDataAsArray', 'entradas');
    const jsonData = response.filter(fact=>fact.almacen === datosEmpresa.empresa.nombre).reverse();
    data.value = jsonData;
};
/************************************************************************/
async function campos() {
  const campos = await arrayToObjetoFromTablaOffline('entradas');
  datoscamposEntradas.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExisteOffline('entradas', camposArray, toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'entradas');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/entradas";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'entradas', JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
    visible.value = false;
    fetchAndSetupData();
...
async function funcionCrear() {
  ...
  if (datoscamposEntradas.value.hasOwnProperty('created_at')) {
    datoscamposEntradas.value.created_at = nfecha('timestamp');
    datoscamposEntradas.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await peticionesFetchOffline('insertData', 'entradas', JSON.stringify(datoscamposEntradas.value));
  if (envioDatos[0] == 'ok') {
    fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await peticionesFetchOffline('deleteEntry', 'entradas', id);
                            } catch (error) {
...
/************************************************************************/
const itemsEntradas = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleEntradas = (event, rowData) => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry', 'entradas', rowData.id);
...
<div class="grid grid-cols-12 gap-4">
      <div class="md:col-span-12">
        <fieldset class="border p-3 rounded mb-2">
          <legend class="float-none w-auto px-2">Datos de Entradas</legend>
            <div class="grid grid-cols-12 gap-4">
...
<Button icon="pi pi-trash" v-if="usuarioLocal.usuario =='Soporte'" raised rounded severity="danger" label="Borrar Todo" class="float-end" @click="borrarTodo" aria-label="Borrar Todo" />
 ...
 <input v-model="searchQuery" placeholder="Buscar entradas..." class="p-inputtext p-component" style="margin-bottom: 10px;" />
</div>
<DataTable 
    :value="filteredEntradas"  
    scrollable 
    scrollHeight="600px"  
    dataKey="id" 
    paginator 
    ...
    <Column header="Options">
        <template #body="slotProps">
            <Button 
                icon="pi pi-cog" 
                @click="toggleEntradas($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
            />
            <Menu 
                ref="menu" 
                id="overlay_menu_Entradas" 
                :model="itemsEntradas" 
                :popup="true"
            />
        </template>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Entradas" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
  ...
  <fieldset class="border p-3 rounded mb-2">
    <legend class="float-none w-auto px-2">Entradas</legend>
     <form id="formularioActualizarEntradas" action="" method="">
...
<input type="input" v-model="datoscampos.cajero" name="cajero"  class="form-control" id="cajero-Actualizador"  placeholder="cajero" v-mayuscula maxlength="250">
</div>
...
<input type="input" v-model="datoscampos.cantidad" name="cantidad"  class="form-control" id="cantidad-Actualizador"  placeholder="cantidad"  maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-4 md:col-span-4 lg:col-span-4 xl:col-span-4" >
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Entradas" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="inline-flex align-items-center justify-content-center gap-2">
...
<input type="input" v-model="datoscamposEntradas.cajero" name="cajero"  class="form-control mayusc" id="cajeroAgregarDatos" v-mayuscula placeholder="cajero" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-4 md:col-span-4 lg:col-span-4 xl:col-span-4" >
<label for="cantidadAgregarDatos">CANTIDAD</label>
<input type="input" v-model="datoscamposEntradas.cantidad" name="cantidad"  class="form-control " id="cantidadAgregarDatos"  placeholder="cantidad" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-4 md:col-span-4 lg:col-span-4 xl:col-span-4" >
<label for="fechaAgregarDatos">FECHA</label>
<input type="input" v-model="datoscamposEntradas.fecha" name="fecha"  class="form-control " id="fechaAgregarDatos"  placeholder="fecha" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-4 md:col-span-4 lg:col-span-4 xl:col-span-4" >
<label for="horaAgregarDatos">HORA</label>
<input type="input" v-model="datoscamposEntradas.hora" name="hora"  class="form-control " id="horaAgregarDatos"  placeholder="hora" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="turnoAgregarDatos">TURNO</label>
<input type="input" v-model="datoscamposEntradas.turno" name="turno"  class="form-control " id="turnoAgregarDatos"  placeholder="turno" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="descripcionAgregarDatos">DESCRIPCION</label>
<textarea class="form-control " v-model="datoscamposEntradas.descripcion" id="descripcionAgregarDatos" name="descripcion" cols="30" rows="3" ></textarea>
</div>
<div class="form-group col-span-12 sm:col-span-6 md:col-span-6 lg:col-span-6 xl:col-span-6" >
<label for="mesAgregarDatos">MES</label>
<input type="input" v-model="datoscamposEntradas.mes" name="mes"  class="form-control " id="mesAgregarDatos"  placeholder="mes" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-6 md:col-span-6 lg:col-span-6 xl:col-span-6" >
<label for="yearAgregarDatos">YEAR</label>
<input type="input" v-model="datoscamposEntradas.year" name="year"  class="form-control " id="yearAgregarDatos"  placeholder="year" maxlength="250">
</div>
<div class="form-group col-span-6" hidden>
<label for="created_atAgregarDatos">CREATED_AT</label>
<input type="input" v-model="datoscamposEntradas.created_at" name="created_at"  class="form-control " id="created_atAgregarDatos"  placeholder="created_at" maxlength="">
</div>
<div class="form-group col-span-6" hidden>
<label for="updated_atAgregarDatos">UPDATED_AT</label>
<input type="input" v-model="datoscamposEntradas.updated_at" name="updated_at"  class="form-control " id="updated_atAgregarDatos"  placeholder="updated_at" maxlength="">
</div>
<div class="form-group col-span-12" hidden>
<label for="usuarioAgregarDatos">USUARIO</label>
<input type="input" v-model="datoscamposEntradas.usuario" name="usuario"  class="form-control " id="usuarioAgregarDatos"  placeholder="usuario" maxlength="250">
</div>
