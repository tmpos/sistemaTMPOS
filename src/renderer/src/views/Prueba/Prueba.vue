...
/************************************************************************/
const camposArray = ['nombre', 'telefono', 'email', 'direccion'];
/************************************************************************/
...
//}else{
  if(window.electron){
     await crearTablaSiNoExisteOffline('prueba',camposArray.join(','),toast)
  }   
//}
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'prueba');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                            const envioDatos = await peticionesFetchOffline('deleteEntry','prueba', id);
                            } catch (error) {
...
/************************************************************************/
const itemsPrueba = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const togglePrueba = (event, rowData) => {
currentRowData.value = rowData;
itemsPrueba.value = [
{ label: 'Editar', icon: 'pi pi-pencil', command: () => { 
router.push({ path: `/editarprueba/${currentRowData.value.id}` });
} },
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry','prueba', rowData.id);
...
const onRowSelect = (event) => {
 router.push({ path: `/editarprueba/${event.data.id}` });
 ...
 <Fieldset legend="Datos de Prueba">
  <div class="flex items-center">
    <div class="flex space-x-2">
      ...
      <Button as="router-link" icon="pi pi-plus-circle" to="/crearprueba" class="ms-1" />
    ...
    <div class="ml-auto">
      ...
      <div class="w-full">
        <div class="flex justify-end mb-4">
          <InputText v-model="searchQuery" placeholder="Buscar prueba..." class="p-inputtext p-component" />
        </div>
        <DataTable
          :value="filteredPrueba"
          scrollable
          scrollHeight="600px"
          dataKey="id"
          paginator
        ...
        <template #body="{ data }">
          ...
          <Column header="Options">
            <template #body="slotProps">
              <Button
                icon="pi pi-cog"
                @click="togglePrueba($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
              />
              <Menu
                ref="menu"
                id="overlay_menu_Prueba"
                :model="itemsPrueba"
                :popup="true"
              />
            </template>
