...
/************************************************************************/
const camposArray = ["nombre","telefono","whatsapp","email","fecha_inicio","direccion","usuario"];
/************************************************************************/
...
const TecnicosEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposTecnicos.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
async function campos() {
  const campos = await arrayToObjetoFromTablaOffline('tecnicos');
  datoscamposTecnicos.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExisteOffline('tecnicos', camposArray, toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await peticionesFetchOffline('deleteAll', 'tecnicos');
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/tecnicos";
  ...
  const envioDatos = await peticionesFetchOffline('updateData', 'tecnicos', JSON.stringify(datoscampos.value));
  if (envioDatos[0] == 'ok') {
    visible.value = false;
    fetchAndSetupData();
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/tecnicos";
  if (datoscamposTecnicos.value.hasOwnProperty('created_at')) {
    datoscamposTecnicos.value.created_at = nfecha('timestamp');
    datoscamposTecnicos.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await peticionesFetchOffline('insertData', 'tecnicos', JSON.stringify(datoscamposTecnicos.value));
  if (envioDatos[0] == 'ok') {
    fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await peticionesFetchOffline('deleteEntry', 'tecnicos', id);
                            } catch (error) {
...
/************************************************************************/
const itemsTecnicos = ref([]);
const menu = ref(null);
const currentRowData = ref(null);
const toggleTecnicos = (event, rowData) => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetchOffline('deleteEntry', 'tecnicos', rowData.id);
...
<div class="grid grid-cols-12 gap-4">
      <div class="md:col-span-12">
        <fieldset class="border p-3 rounded mb-2">
          <legend class="float-none w-auto px-2">Datos de Tecnicos</legend>
            <div class="grid grid-cols-12 gap-4">
...
<a href="#" v-if="usuarioLocal.usuario =='Soporte'"  @click="borrarTodo" class="btn btn-danger btnaccion cartelito float-end ms-1" data-toggle="tooltip" title="Borrar Todo" id="borrartodo"><i class="icon-trash"></i> Borrar Todo</a>
 ...
 <input v-model="searchQuery" placeholder="Buscar tecnicos..." class="p-inputtext p-component" style="margin-bottom: 10px;" />
</div>
<DataTable
    ...
    <Column header="Options">
        <template #body="slotProps">
            <Button
                icon="pi pi-cog"
                @click="toggleTecnicos($event, slotProps.data)"
                aria-haspopup="true"
                aria-controls="overlay_menu_factura"
            />
            <Menu
                ref="menu"
                id="overlay_menu_Tecnicos"
                :model="itemsTecnicos"
                :popup="true"
            />
        </template>
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visible" :position="position" modal header="Modificar Tecnicos" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
  ...
  <fieldset class="border p-3 rounded mb-2">
    <legend class="float-none w-auto px-2">Tecnicos</legend>
     <form id="formularioActualizarTecnicos" action="" method="">
...
<div class="form-group col-span-12 sm:col-span-3 md:col-span-3 lg:col-span-3 xl:col-span-3" >
<label for="fecha_inicio-Actualizador">FECHA_INICIO</label>
<input type="input" v-model="datoscampos.fecha_inicio" name="fecha_inicio"  class="form-control" id="fecha_inicio-Actualizador" v-datepicker placeholder="fecha_inicio"  maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
...
<!-- ////////////////////////////////////////////////////////////////////////////////////////// -->
<Dialog v-model:visible="visiblecrear" :position="position" modal header="Crear Tecnicos" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="inline-flex align-items-center justify-content-center gap-2">
...
<input type="input" v-model="datoscamposTecnicos.nombre" name="nombre"  class="form-control mayusc" id="nombreAgregarDatos" v-mayuscula placeholder="nombre" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-3 md:col-span-3 lg:col-span-3 xl:col-span-3" >
<label for="telefonoAgregarDatos">TELEFONO</label>
<InputMask id="telefonoAgregarDatos" class="form-control" v-model="datoscamposTecnicos.telefono" :mask="patronTelefono" :placeholder="patronTelefono" />
</div>
<div class="form-group col-span-12 sm:col-span-3 md:col-span-3 lg:col-span-3 xl:col-span-3" >
<label for="whatsappAgregarDatos">WHATSAPP</label>
<InputMask id="whatsappAgregarDatos" class="form-control" v-model="datoscamposTecnicos.whatsapp" :mask="patronTelefono" :placeholder="patronTelefono" />
</div>
<div class="form-group col-span-12 sm:col-span-3 md:col-span-3 lg:col-span-3 xl:col-span-3" >
<label for="emailAgregarDatos">EMAIL</label>
<input type="input" v-model="datoscamposTecnicos.email" name="email"  class="form-control " id="emailAgregarDatos"  placeholder="email" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-3 md:col-span-3 lg:col-span-3 xl:col-span-3" >
<label for="fecha_inicioAgregarDatos">FECHA_INICIO</label>
<input type="input" v-model="datoscamposTecnicos.fecha_inicio" name="fecha_inicio"  v-datepicker  class="form-control " id="fecha_inicioAgregarDatos"  placeholder="fecha_inicio" maxlength="250">
</div>
<div class="form-group col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-12 xl:col-span-12" >
<label for="direccionAgregarDatos">DIRECCION</label>
<textarea class="form-control " v-model="datoscamposTecnicos.direccion" id="direccionAgregarDatos" name="direccion" cols="30" rows="3" ></textarea>
</div>
<div class="form-group col-span-6" hidden>
<label for="created_atAgregarDatos">CREATED_AT</label>
<input type="input" v-model="datoscamposTecnicos.created_at" name="created_at"  class="form-control " id="created_atAgregarDatos"  placeholder="created_at" maxlength="">
</div>
<div class="form-group col-span-6" hidden>
<label for="updated_atAgregarDatos">UPDATED_AT</label>
<input type="input" v-model="datoscamposTecnicos.updated_at" name="updated_at"  class="form-control " id="updated_atAgregarDatos"  placeholder="updated_at" maxlength="">
</div>
<div class="form-group col-span-12" hidden>
<label for="usuarioAgregarDatos">USUARIO</label>
<input type="input" v-model="datoscamposTecnicos.usuario" name="usuario"  class="form-control " id="usuarioAgregarDatos"  placeholder="usuario" maxlength="250">
</div>
