...
function guardarConfiguracion() {
  ...
  localStorage.setItem('configuracionTema', JSON.stringify(config));
}
...
/***************************************************/
onMounted(async () => {
  const configLocal = JSON.parse(localStorage.getItem('configuracionTema'));

  if (configLocal) {
    // Aplicar configuración guardada
    preset.value = configLocal.preset || 'Aura';
    menuMode.value = configLocal.menuMode || 'static';

    setPreset(preset.value);
    setMenuMode(menuMode.value);
    setPrimary(configLocal.primary || 'blue');
    setSurface(configLocal.surface || 'slate');

    await nextTick(); // Esperar que layoutConfig se actualice visualmente

    const surfacePalette = surfaces.value.find((s) => s.name === configLocal.surface)?.palette;
    ...
    // No hay configuración previa, usar la actual y guardarla
    preset.value = layoutConfig.preset;
    menuMode.value = layoutConfig.menuMode;

    setPreset(preset.value);
    setMenuMode(menuMode.value);
    setPrimary(layoutConfig.primary);
    setSurface(layoutConfig.surface);
