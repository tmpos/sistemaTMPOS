...
export async function imprimirTicketCotizacion(factura, datosEmpresa) {
...
const mesa = datosOtro[0].mesa;*/
  ...
  const qrCodeData = await QRCode.toDataURL(
    `${link}/receipt/ticket?cotizacion=${datosFactura.no_cotizacion}`
  )
        ...
        <div class="ticket">
            ...
            <div id="mid" class="bordeado">
               ...
               <p>
                    ...
                    ${configuracionfactura.no_factura ? `DOC: <b>#${datosFactura.no_cotizacion}</b><br>` : ''}
                    ${configuracionfactura.nombre_cliente ? `CLIENTE: ${datosFactura.nombre_cliente || 'SIN REGISTRO'}<br>` : ''}
                    ${configuracionfactura.direccion_cliente ? `DIRECCION: ${datosFactura.direccion_cliente || 'SIN REGISTRO'}<br>` : ''}
                    ${configuracionfactura.vendedor ? `VENDEDOR: ${datosFactura.vendedor}<br>` : ''}
                    VENCIMIENTO: ${datosFactura.vencimiento}
                </p>
                </div>
            </div>
            ...
            <div id="subtotal" style="font-weight: bold;">
                <table>
                    <tr>
                        <td>SUBTOTAL:</td>
                        <td style="text-align:right;"><span style="font-size: 1.5em !important;margin-top: 5px;margin-bottom: 5px;">${datosConfiguracion.simbolo}${(Number(datosFactura.total) + Number(datosFactura.descuento)).toFixed(2)}</span></td>
                    </tr>
                </table>
            </div>
