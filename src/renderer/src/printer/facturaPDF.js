...
export async function pdfFacturaCarta(factura, cliente, datosEmpresa, datosCredito) {
  ...
  if (datosFactura.metodo_pago === 'CREDITO') {
    if (datosCredito) {
      const creditoFact = JSON.parse(datosCredito)

      abonado = Number(datosFactura.total) - Number(creditoFact.saldo)
      pendiente = creditoFact.saldo
    } else {
  ...
  const productosHTML = productos
    .map((producto) => {
      ...
      //const totalProducto = ((parseFloat(producto.precio_final) * parseFloat(producto.cantidad)) - parseFloat(producto.descuento));

      const precio = parseFloat(producto.precio_final) || 0
      const cantidad = parseFloat(producto.cantidad) || 0
      const descuento = parseFloat(producto.descuento) || 0
      const totalProducto = precio * cantidad - descuento

      const totalImpuestos = parseFloat(producto.impuesto) * parseFloat(producto.cantidad)
      return `
        <tr>
          ...
          ${datosDefault.impuestos ? `<td class=" text-right pr-2">${formatoMonedaRD(totalImpuestos)}</td>` : ''}
        ${datosDefault.descuento ? `<td class=" text-right pr-2">${formatoMonedaRD(Number(producto.descuento))}</td>` : ''}
...
<body class="p-4 text-xs font-sans">
  <div class="max-w-3xl mx-auto  p-4">
    ...
    <div class="border border-black flex justify-between items-start mt-4 rounded-md">
     ...
     ${datosDefault.nombre_cliente ? `<p><strong>CLIENTE:</strong>  ${datosFactura.nombre_cliente || 'SIN REGISTRO'}</p>` : ''}
     ${datosDefault.telefono_cliente ? `<p><strong>TELEFONO:</strong>  ${datosCliente.telefono || 'N/A'}</p>` : ''}
     ${datosDefault.rnc ? `<p><strong>RNC/CEDULA:</strong>  ${datosCliente.rnc || 'N/A'}</p>` : ''}
     ${datosDefault.direccion_cliente ? `<p><strong>DIRECCIÓN:</strong>  ${datosCliente.direccion || 'N/A'}</p>` : ''}
      ...
      <thead class="border border-black bg-gray-800 text-white">
        <tr>
          ...
          <th class="border border-black px-1 py-1">P.U</th>
           ${datosDefault.impuestos ? `<th class="border border-black px-1 py-1" >${datosConfiguracion.nombre_impuesto}</th>` : ''}
    ...
    <div class=" w-1/3">
        ...
        <div class=" w-1/3 rounded-lg border shadow-sm">
          <table class="text-right text-sm w-full total"  >
            ...
            ${datosDefault.impuestos ? `<tr><td class=" px-1">${datosConfiguracion.nombre_impuesto} (${datosConfiguracion.impuesto}%)</td><td class=" px-1">${formatoMonedaRD(datosFactura.impuesto)}</td></tr>` : ''}
            ${datosDefault.descuento ? `<tr><td class=" px-1">DESC.</td><td class=" px-1">${formatoMonedaRD(datosFactura.descuento)}</td></tr>` : ''}
