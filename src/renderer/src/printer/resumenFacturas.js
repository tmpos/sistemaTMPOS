...
export async function imprimirReporteFacturas(datosLoL, datosEmpresa, silent, visible, show) {
  /************************************************************/
  ...
  const productosHTML = datosArray
    .map((factura) => {
      return `
        <tr>
            <td style="text-align:left; padding: 5px;">${factura.no_factura}</td>
            <td style="text-align:left; padding: 5px;">${factura.fecha_emision}</td>
            <td style="text-align:left; padding: 5px;">${factura.hora}</td>
            <td style="text-align:left; padding: 5px;">${factura.metodo_pago.substring(0, 4)}</td>
            <td style="text-align:left; padding: 5px;">${factura.total}</td>
        </tr>
    `
    })
    .join('')

  const totalVendido = datosArray.reduce((acc, factura) => acc + Number(factura.total), 0)
