...
export async function imprimirTicketPreFactura(factura, datosEmpresa) {
  ...
  //const datosFactura = await peticionesFetch(`${link}${api}`,`datoscampo/pre_facturas/no_factura/${factura}`,{},tokenCifrado,'GET');
  /**********************************************************************/
...
const mesa = datosOtro[0].mesa;*/
  ...
  const qrCodeData = await QRCode.toDataURL(
    `${link}/receipt/ticket?prefactura=${datosFactura.no_factura}`
  )
        ...
        <div class="ticket">
            ...
            <div id="mid" class="bordeado">
               ...
               <p>
                    ...
                    ${configuracionfactura.no_factura ? `DOC: <b>#${datosFactura.no_factura}</b><br>` : ''}
                    ${configuracionfactura.nombre_cliente ? `CLIENTE: ${datosFactura.cliente || 'SIN REGISTRO'}<br>` : ''}
                    ESTADO: ${datosFactura.estado}
                </p>
                </div>
            </div>
            <div  class="bordeado" style="text-align:center;padding:3px;background-color:#ffeb3b;font-weight:bold;">
  ...
  console.log('✅ Pre-Factura Ticket 80mm cargada en ventana')
}
