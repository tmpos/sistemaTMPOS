...
export async function imprimirReporteImei(datosLoL, datosEmpresa, silent, visible, show) {
  /************************************************************/
  ...
  const productosHTML = datosArray
    .map((factura) => {
      return `
        <tr>
            <td style="text-align:left; padding: 5px;">${factura.imei}</td>
            <td style="text-align:left; padding: 5px;">${factura.equipo}</td>
            <td style="text-align:left; padding: 5px;">${factura.comprador}</td>
            <td style="text-align:left; padding: 5px;">${factura.fecha_venta} ${factura.hora_venta}</td>
        </tr>
    `
    })
    .join('')
