...
export async function imprimirTicketApartado(factura, datosEmpresa) {
  ...
  //const datosApartado = await peticionesFetch(`${link}${api}`,`datoscampo/apartados/no_apartado/${factura}`,{},tokenCifrado,'GET');
  /**********************************************************************/
  ...
  //const datosOtro = JSON.parse(datosApartado.otro)
...
const mesa = datosOtro[0].mesa;*/
  ...
  const qrCodeData = await QRCode.toDataURL(
    `${link}/receipt/ticket?apartado=${datosApartado.no_emision}`
  )
    ...
    pagosRealizados.length > 0
      ? pagosRealizados
        ...
        <tr>
            ...
            <td class="precio centrado">${pago.fecha || ''} ${pago.hora || ''}</td>
            <td class="precio centrado">${pago.metodo || 'EFECTIVO'}</td>
            <td style="text-align:right;">${datosConfiguracion.simbolo}${Number(pago.cantidad || 0).toFixed(2)}</td>
            <td style="text-align:right;">${datosConfiguracion.simbolo}${Number(pago.saldo || 0).toFixed(2)}</td>
        </tr>
    `
          )
          .join('')
    ...
    <body>
        <div class="ticket">
            ...
            <div id="mid" class="bordeado">
                <div class="info ">
               <p>
                    ${configuracionfactura.fecha ? `Fecha: ${datosApartado.fecha_emision}<br>` : ''}
                    ${configuracionfactura.no_factura ? `DOC: <b>#${datosApartado.no_emision}</b><br>` : ''}
                    ${configuracionfactura.nombre_cliente ? `CLIENTE: ${datosApartado.cliente || 'SIN REGISTRO'}<br>` : ''}
                    ESTADO: ${datosApartado.estatus}
                </p>
                </div>
            </div>
            <div  class="bordeado" style="text-align:center;padding:3px;background-color:#ff9800;color:white;font-weight:bold;">
            ...
            <div id="total" style="font-weight: bold;">
                <table>
                    <tr>
                        <td>TOTAL:</td>
                        <td style="text-align:right;"><span style="font-size: 1.5em !important;margin-top: 5px;margin-bottom: 5px;">${datosConfiguracion.simbolo}${datosApartado.monto_credito}</span></td>
                    </tr>
                    <tr>
                        <td>ABONADO:</td>
                        <td style="text-align:right;"><span style="font-size: 1.5em !important;margin-top: 5px;margin-bottom: 5px;">${datosConfiguracion.simbolo}${datosApartado.abonado}</span></td>
                    </tr>
                    <tr>
                        <td>PENDIENTE:</td>
                        <td style="text-align:right;"><span style="font-size: 1.5em !important;margin-top: 5px;margin-bottom: 5px;">${datosConfiguracion.simbolo}${datosApartado.saldo}</span></td>
                    </tr>
                </table>
            </div>
            ...
            <div class="bordeado" style="margin-top: 10px;">
                <center style="font-weight: bold; padding: 5px;">PAGOS REALIZADOS</center>
                <table cellspacing="0" cellpadding="0">
                    <thead class="linea">
                        <tr>
                            <th class="" style="text-align:left;padding: 3px;">#</th>
                            <th class="precio" style="text-align:center;padding: 3px;">FECHA</th>
                              <th class="precio" style="text-align:center;padding: 3px;">MÉTODO</th>
                            <th class="precio" style="text-align:right;padding: 3px;">MONTO</th>
                            <th class="precio" style="text-align:right;padding: 3px;">SALDO</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${pagosHTML}
  ...
  console.log('✅ Apartado Ticket 80mm cargada en ventana')
}
